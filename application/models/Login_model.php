...
class Login_model extends CI_Model
{
     ...
     */
    function loginMe($email, $password, $roleId)
    {
        $this->db->select('BaseTbl.userId, BaseTbl.password, BaseTbl.name, BaseTbl.roleId, BaseTbl.is_hr, BaseTbl.email_verified, BaseTbl.mobile_verified, BaseTbl.company_id, BaseTbl.status, Roles.role');
     ...
     */
    function verifyEmail($userId, $email)
    {
        ...
        if ($query->num_rows() > 0){
          $this->db->flush_cache();
          $this->db->where('userId', $userId);
          return $this->db->update('tbl_users', array('email_verified' => 1, 'status' => 1));
        } else {
            return false;
        }
    }
    ...
    {
        $this->db->select('userId, email,fname, lname, name, roleId, password');
