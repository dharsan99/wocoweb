...
class Manager_model extends CI_Model{
  ...
  function getManagerList($companyId = "", $userId = "")
  {
     ...
     $columns = array(
      ...
      $query = $this->db->select('*')->from('tbl_users')->where('roleId', ROLE_MANAGER)->where('company_id', $companyId)->where('userId !=', $userId)->get();
      ...
      $this->db->select('*');
      $this->db->from('tbl_users');
      $this->db->where('roleId', ROLE_MANAGER)->where('company_id', $companyId)->where('userId !=', $userId);
