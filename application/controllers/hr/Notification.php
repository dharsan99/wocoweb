...
class Notification extends HrController{

  public function __construct()
  {
    parent::__construct();
    //Codeigniter : Write Less Do More
    $this->load->model(array('hr/notification_model'));
    $this->isLoggedIn();
  ...
  public function index()
  {
      ...
      $this->loadViews("notification/listData", $this->global, $data, NULL);
  }

  public function getNotificationList()
  {
    $json_data = $this->notification_model->getNotificationList($this->companyId);
  ...
  function addNew()
  {
    ...
    $result = $this->notification_model->createEmptype($dataArr);
    if ($result>0) {
    ...
    function editEmptype($id = "")
    {
        ...
        else
        {
            ...
            {
                if (!$this->notification_model->isValidEmptype($id, $this->companyId)) {
                  echo json_encode(array('status' => 0, 'message' => 'Access Denied' ));
                  die();
                }
                $data = $this->notification_model->getEmptypeInfo($id);
            ...
            else
            {
              ...
              $description = $this->input->post('description');
                ...
                $result = $this->notification_model->editEmptype($emptypeInfo, $id);
                if ($result) {
    ...
    function deleteEmptype($id="")
    {
        ...
        {
            $result = $this->notification_model->deleteEmptype($id);
    ...
    function blockAdmin($userId="")
    {
        ...
        {
          if (!$this->notification_model->isValidUserId($userId, $this->companyId)) {
            echo json_encode(array('status' => 0, 'message' => 'Access Denied' ));
            die();
          }
            $adminInfo = array('status'=>2);
            $result = $this->notification_model->blockAdmin($userId, $adminInfo);
    ...
    function unblockAdmin($userId="")
    {
        ...
        else
        {
          if (!$this->notification_model->isValidUserId($userId, $this->companyId)) {
            ...
            $result = $this->notification_model->unblockAdmin($userId, $adminInfo);
