...
class Manager extends HrController{

  public function __construct()
  {
    parent::__construct();
    //Codeigniter : Write Less Do More
    $this->load->model(array('hr/manager_model'));
    $this->isLoggedIn();
  ...
  public function index()
  {
      ...
      $this->global['pageTitle'] = 'WoCo : Manager Listing';
      $this->loadViews("manager/listData", $this->global, $data, NULL);
  }

  public function getManagerList()
  {
    $json_data = $this->manager_model->getManagerList($this->companyId, $this->vendorId);
    ...
    function addNew()
    {
      ...
      if (!$this->manager_model->isValidCompany($company_id)) {
        echo json_encode(array('status' => 0, 'message' => "Your company doesn't exist or deactivated." ));
        die();
      }
      if (!$this->manager_model->isValidEmpId($emp_id, $company_id)) {
        echo json_encode(array('status' => 0, 'message' => "Access Denied" ));
        die();
      }

      $result = $this->manager_model->edit( array('roleId' => ROLE_MANAGER), $emp_id);
      if ($result) {
        $this->acl_model->insertAllUserPermissions($emp_id, ROLE_MANAGER, $company_id, $this->vendorId);
        $userInfo = $this->manager_model->getManagerInfo($emp_id);
  ...
  function createNew()
  {
    ...
    if ($this->manager_model->isEmailExisting($email)) {
      echo json_encode(array('status' => 0, 'message' => "Entered email alredy registered." ));
      die();
    }
    if ($this->manager_model->isMobileExisting($mobile)) {
    ...
    $dataArr = array(
      ...
      'manager_id' => $this->common_model->getCompanyHrId($this->companyId),
      'joining_date' => date('Y-m-d H:i:s'),
      'password' => getHashedPassword($password)
    );

    $result = $this->manager_model->createManager($dataArr);
    if ($result > 0) {
    ...
    function edit($id = "")
    {
        ...
        else
        {
            ...
            {
                if (!$this->manager_model->isValidUserId($id, $this->companyId)) {
                  echo json_encode(array('status' => 0, 'message' => 'Access Denied' ));
                  die();
                }
                $data = $this->manager_model->getManagerInfo($id);
                if (count($data)>0) {
            ...
            //  $password   = generate_password(8);
              ...
              if (!$this->manager_model->isValidCompany($company_id, $id)) {
                echo json_encode(array('status' => 0, 'message' => "Your company doesn't exist or deactivated." ));
                die();
              }
              if ($this->manager_model->isEditEmailExisting($email, $id)) {
                echo json_encode(array('status' => 0, 'message' => "Entered email alredy registered." ));
                die();
              }
              if ($this->manager_model->isEditMobileExisting($mobile, $id)) {
                echo json_encode(array('status' => 0, 'message' => "Entered mobile number alredy registered." ));
                die();
              }
                ...
                $result = $this->manager_model->edit($hrInfo, $id);
                if ($result) {
    ...
    function deleteManager($userId="")
    {
        ...
        {
          if (!$this->manager_model->isValidUserId($userId, $this->companyId)) {
            echo json_encode(array('status' => 0, 'message' => 'Access Denied' ));
            die();
          }
            $result = $this->manager_model->deleteManager($userId);
    ...
    function blockManager($userId="")
    {
        ...
        else
        {
          if (!$this->manager_model->isValidUserId($userId, $this->companyId)) {
            ...
            $result = $this->manager_model->blockManager($userId, $HrInfo);
    ...
    function unblockManager($userId="")
    {
        ...
        else
        {
          if (!$this->manager_model->isValidUserId($userId, $this->companyId)) {
            ...
            $result = $this->manager_model->unblockManager($userId, $HrInfo);
    ...
    public function employeeSearch()
    {
        ...
        $result = $this->manager_model->searchEmployee($searchText, $this->companyId);
        $totalData = array();
