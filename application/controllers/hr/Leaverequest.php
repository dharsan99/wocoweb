...
class Leaverequest extends HrController{

  public function __construct()
  {
    parent::__construct();
    //Codeigniter : Write Less Do More
    $this->load->model(array('hr/leaverequest_model'));
    $this->isLoggedIn();
  ...
  public function index()
  {
      ...
      $data['test'] = 'test';
      $data['department']      = $this->leaverequest_model->getDepartmentList($this->companyId);
      $this->global['pageTitle'] = 'WoCo : Team Listing';
      $this->loadViews("leaverequest/listData", $this->global, $data, NULL);
  }

  public function getLeaverequestList()
  {
    $json_data = $this->leaverequest_model->getLeaverequestList($this->companyId);
  ...
  function addNew()
  {
    ...
    $result = $this->leaverequest_model->createTeam($dataArr);
    if ($result>0) {
     ...
     */
     function getuserbydeaprtmentid($department_id="")
     {
           if($this->isHr() == TRUE)
           {
            ...
            $data['head']      = $this->leaverequest_model->getDepartmentList($this->companyId);
           $data = $this->leaverequest_model->getuserbydeaprtmentid($department_id, $this->companyId);
           if (count($data)>0) {
    ...
    function editTeam($id = "")
    {
        ...
        else
        {
            ...
            {
                if (!$this->leaverequest_model->isValidTeam($id, $this->companyId)) {
                  echo json_encode(array('status' => 0, 'message' => 'Access Denied' ));
                  die();
                }
                $data = $this->leaverequest_model->getTeamInfo($id);
            ...
            else
            {
              ...
              $description = $this->input->post('description');
                ...
                $result = $this->leaverequest_model->editTeam($teamInfo, $id);
                if ($result) {
    ...
    function deleteTeam($id="")
    {
        ...
        {
            $result = $this->leaverequest_model->deleteTeam($id);
    ...
    function blockTeam($userId="")
    {
        ...
        {
          if (!$this->leaverequest_model->isValidUserId($userId, $this->companyId)) {
            echo json_encode(array('status' => 0, 'message' => 'Access Denied' ));
            die();
          }
            $teamInfo = array('status'=>2);
            $result = $this->leaverequest_model->blockTeam($userId, $teamInfo);
    ...
    function unblockTeam($userId="")
    {
        ...
        else
        {
          if (!$this->leaverequest_model->isValidUserId($userId, $this->companyId)) {
            ...
            $result = $this->leaverequest_model->unblockTeam($userId, $adminInfo);
